...
class Product
{
    ...
    public function search($str)
    {
        ...
        $query = "SELECT DISTINCT p.id, p.name, p.price, p.boxId, p.coefficient FROM Product p WHERE p.name LIKE '$str%' UNION SELECT DISTINCT p.id, p.name, p.price, p.boxId, p.coefficient FROM Product p WHERE p.name LIKE '%$str%'";
        ...
        while ($p = $stmt->fetch()) {
            ...
            $dc = $this->category->readParentCategory($this->category->readFirst($p['id'])['id']);
            $p['discountCategoryId'] = $dc['id'];
            $p['categoryId'] = $c['id'];
